...
namespace WindowsFormsApp2
{
    public partial class Form1 : Form
    {
            ...
            if (PrintStr < Count)
            {
                for (int i = 0; i < this.Count; i++)
                {
                    ...
                    if (i != 0 && i != 1 && i != Count - 2 && i != Count - 1)
                    {
                        ...
                        if (Convert.ToInt32(LifeRule[ALive]) == '1')
                        {
                            TempData[i] = 1;
                        }
                        else TempData[i] = 0;
                ...
                for (int i = 0; i < Count; i++)
                {
                    if (TempData[i] == 0)
                    {
                        g.FillRectangle(Brushes.Black, BordPixNum[i], BordPixNum[PrintStr], this.Scale, this.Scale);
                    }
                }
                for (int i = 0; i < Count; i++)
                {
        ...
        private void Button2_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < Count; i++)
            {
                InputData[i] = rnd.Next(0, 2);
            }
            for (int i = 0; i < Count; i++)
            {
                if (InputData[i] == 0)
                {
                    g.FillRectangle(Brushes.Black, BordPixNum[i], 0, this.Scale, this.Scale);
                }
            }
            this.PrintStr = 0;
